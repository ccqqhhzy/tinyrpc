...
#ifndef __CLIENT_CC_H__
#define __CLIENT_CC_H__
...
class CcClient : public RpcClient<CcClient> {
public:
    using RpcClient::RpcClient;
    explicit CcClient(const ClientOptions& opt);
...
template<typename REQ, typename RSP>
bool CcClient::registerCallback(std::function<void(
...
template<typename REQ, typename RSP>
bool CcClient::synCall(const REQ& req, std::shared_ptr<RSP>& rsp, 
                       uint32_t timeout) {
    if (!conn_ || !codec_) {
        return false;
    }
...
template<typename REQ>
bool CcClient::asynCall(const REQ& req) {
    std::string message;
    if (!serialize(req, message)) {
        LOG(Error, "Serialize failed, uri:0x%xu", REQ::URI);
        return false;
